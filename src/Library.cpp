...
void Library::listBooks() const
{
  ...
  {
    auto b = kv.second;
    std::cout << "- ISBN: " << b->getIsbn() << " | Title: " << b->getTitle() << " | Author: " << b->getAuthor()
    ...
    for (const auto& item : user->getBorrowedBooks())
    {
      ...
      if (days > user->getMaxDays())
      {
        ...
        std::cout << "- User: " << user->getId() << " | Name: " << user->getName()
                  << " | ISBN: " << item.book->getIsbn() << " | Title: " << item.book->getTitle()
                  << " | Borrowed on: " << item.borrowDate << " | Days borrowed: " << days
