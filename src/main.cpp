...
int main() {
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            std::string isbn;
            std::cout << "ISBN to remove: "; std::getline(std::cin, isbn);
            library.removeBook(isbn);
            std::cout << "Book removed (if existed).\n";
        } else if (choice == 3) {
            std::string id, name; int type;
            std::cout << "User ID: "; std::getline(std::cin, id);
            std::cout << "Name: "; std::getline(std::cin, name);
            std::cout << "Type (1=Student,2=Faculty,3=Guest): "; std::cin >> type;
            std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
            if (type == 1) library.addUser(std::make_shared<Student>(id, name));
            else if (type == 2) library.addUser(std::make_shared<Faculty>(id, name));
            else library.addUser(std::make_shared<Guest>(id, name));
            std::cout << "User registered.\n";
        } else if (choice == 4) {
            std::string userId, isbn, date;
            std::cout << "User ID: "; std::getline(std::cin, userId);
            std::cout << "ISBN: "; std::getline(std::cin, isbn);
            std::cout << "Borrow date (YYYY-MM-DD): "; std::getline(std::cin, date);
            if (library.borrowBook(userId, isbn, date)) std::cout << "Book borrowed.\n";
            else std::cout << "Failed to borrow book.\n";
        } else if (choice == 5) {
            std::string userId, isbn;
            std::cout << "User ID: "; std::getline(std::cin, userId);
            std::cout << "ISBN: "; std::getline(std::cin, isbn);
            if (library.returnBook(userId, isbn)) std::cout << "Book returned.\n";
            else std::cout << "Failed to return book.\n";
        } else if (choice == 6) {
            library.listBooks();
        } else if (choice == 7) {
            library.listUsers();
